...
if __name__ == "__main__":
    ...
    H2_PeakTimeAmp = [ROOT.TH2F(f"PeakTimeAmp_ch{i+1}", f"CH{i+1} time vs ADC_peak", nADC, 0, nADC, 2000 + 10, -10, 2000) for i in range(nCh)]
    ...
    for b in range(2):
        ...
        for c in range(2):
            ...
            if c == 0:
                ...
                PedMax = H1_Pedestal[0].GetMaximum()
                PedFrame = ROOT.gPad.DrawFrame(300, 0.5, 500, PedMax * 1.3)
                ROOT.gPad.SetLogy()
                PedFrame.GetXaxis().SetLabelSize(0.05)
                PedFrame.GetYaxis().SetLabelSize(0.05)
                PedFrame.GetXaxis().SetTitleSize(0.05)
                PedFrame.GetXaxis().SetTitle(f"CH{b+1} Pedestal [ADC]")  # f-string을 사용하여 채널 이름 설정
                PedFrame.Draw()

                H1_Pedestal[b].SetLineColor(1)
                H1_Pedestal[b].Draw("SAME")
